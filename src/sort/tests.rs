...
mod tests {
    ...
    async fn test_sort() {
        ...
        let mut cpu_pairs: Vec<(u32, u32)> = values.iter().copied().zip(keys.iter().copied()).collect();
        cpu_pairs.sort_by_key(|&(v, _)| v);
        let (cpu_values, cpu_keys): (Vec<u32>, Vec<u32>) = cpu_pairs.into_iter().unzip();

        let (gpu_values, gpu_keys) = sorter.sort_array(&ctx.device, &ctx.queue, &values, &keys).await;
